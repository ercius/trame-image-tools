...
class ExampleApp:
    ...
    @change("roi")
    def update_zoomed_image(self, *args, **kwargs):
        roi = self.state.roi
        zoomed_image = self.image.crop((roi[0], roi[1], roi[0] + roi[2], roi[1] + roi[3]))
        self.state.zoomed_image = self.convert_to_base64(zoomed_image)
        self.state.zoomed_image_size = zoomed_image.size
    ...
    def _build_ui(self):
        with DivLayout(self.server) as layout:
            ...
            with html.Div(
                style="position: absolute; width: 50%; height: 100%; background-color: black;"
            ):
                with TrameImage(
                    src=("image",),
                    size=("image_size",),
            ...
            with html.Div(
                ...
                TrameImage(
                    ...
                    v_model_center=("center_right", [0.5, 0.5]),
                )
